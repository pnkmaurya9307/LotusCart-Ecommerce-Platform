        ...
        <div className='lg:w-[85%] md:w-[70%] h-[100%] lg:ml-[310px] md:ml-[250px] mt-[70px] flex flex-col gap-[30px] overflow-x-hidden py-[50px] ml-[100px]'>
          <div className='w-[400px] h-[50px] text-[28px] md:text-[40px] mb-[20px] text-white'>All Orders List</div>
          {
           orders.map((order,index)=>(
            ...
            <img src={order.items[0]?.image1} alt="" className='w-[60px] h-[60px] rounded-lg object-cover'/>

            <div className='flex items-start justify-center flex-col gap-[8px]'>
              {
                order.items.map((item,index)=>(
                  <p key={index} className='text-[16px] text-[#56dbfc]'>{item.name.toUpperCase()}  *  {item.quantity} <span>{item.size}</span>{index !== order.items.length - 1 ? "," : ""}</p>
                ))
              }
